...
async function forgotPassword(req, res) {
  ...
  // If the user exists, generate a code and send it to their real email.
  const [user] = await db.query('SELECT id FROM users WHERE email = ?', [email]);

  if (user?.id) {
    const code = await createPasswordResetCodeForUser(user.id);
...
/**
 * Reset password using email + verification code generated by `forgotPassword`.
 */
async function resetPassword(req, res) {
  ...
  try {
    ...
    await conn.query(
      'UPDATE password_reset_codes SET used = 1, used_at = NOW() WHERE id = ?',
      [record.id]
    );
    await conn.commit();
  } catch (e) {
...
async function createPasswordResetCodeForUser(userId) {
  // Invalidate previous unused codes for this user.
  await db.query('DELETE FROM password_reset_codes WHERE user_id = ? AND used = 0', [userId]);
  ...
  await db.query(
    `INSERT INTO password_reset_codes (user_id, code_hash, expires_at, used)
     VALUES (?, ?, ?, 0)`,
